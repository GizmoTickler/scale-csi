...
CSI spec v1.13 removed the alpha `VolumeCondition` field, so this driver no
longer reports per-volume health through CSI and the chart no longer ships the
external-health-monitor sidecar or the `backendHealth` pool poller (both keys are
still accepted and ignored). `NodeGetVolumeStats` still refuses to stat a stale
mount: it runs a bounded mount check first and returns `Internal` when the mount
is unresponsive, so a dead NFS hard mount cannot hang kubelet's stats loop.
