...
| `backendHealth.enabled` | Run a controller-only **read-only** pool-health poller (`pool.query` + `disk.temperature_alerts`) that fans pool health onto every managed PVC's `VolumeCondition` and exports the `scale_csi_pool_status`/`_healthy`/`_scan_state`/`_scan_errors`/`_disk_temp_alerts`/`_disk_temp_alerts_age_seconds`/`_health_stale`/`_health_flip_pending` gauges. The temperature count is paired with its actual component age; an absent age means no successful temperature result has returned yet. The gauges are raw, the condition is debounced, the alerts add their own `for` hold and Prometheus observes all of it on its own cadence, so they differ transiently in seven numbered classes — 1. confirmation lag (**bounded**, and it bounds driver-side publication only), 2. alert hold (**bounded**), 3. recovery (**bounded**), 4. poll stall (**unbounded**, it lasts until a successful usable sample arrives), 5. observer lag (**unbounded**, scrape/rule-evaluation cadence), 6. cold start (**unbounded**, before the first successful sample the raw series do not exist, and the stale gauge cannot tell a missing pool from an unreachable one), 7. replica skew (**unbounded**, a producer-identity difference: every replica polls, so `max by (pool)` can merge two histories). `controller.replicas` must be exactly 1; Helm rejects a larger value. The skew alert detects, but cannot prevent, Deployment termination/drain/eviction overlap. It also exports `scale_csi_pool_health_last_success_timestamp_seconds`, the driver-owned wall-clock sample time (process-local, reset on restart) to use instead of PromQL `timestamp()`. See "Signal timing" in `docs/production.md`. Default off |
| `backendHealth.interval` | Health poll cadence; default `60s`, clamped to `30s`–`2m` (the ceiling keeps a hysteresis-confirmed condition flip inside the 5m `ScaleCSIPoolDegraded` hold **while successful samples keep arriving** — it bounds cadence, not wall-clock publication, and it creates the alert-hold class rather than removing it). Staleness TTL is 3 x the effective clamped value |
