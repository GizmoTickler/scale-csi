...
| `backendHealth.enabled` | Run a controller-only **read-only** pool-health poller (`pool.query` + `disk.temperature_alerts`) that fans pool health onto every managed PVC's `VolumeCondition` and exports the `scale_csi_pool_status`/`_healthy`/`_scan_state`/`_scan_errors`/`_disk_temp_alerts`/`_health_stale`/`_health_flip_pending` gauges. The gauges are raw, the condition is debounced, the alerts add their own `for` hold and Prometheus observes all of it on its own cadence, so they differ transiently in seven numbered classes — 1. confirmation lag (**bounded**, and it bounds driver-side publication only), 2. alert hold (**bounded**), 3. recovery (**bounded**), 4. poll stall (**unbounded**, until a successful usable sample arrives), 5. observer lag (**unbounded**, scrape/rule-evaluation cadence), 6. cold start (**unbounded**, before the first successful sample the raw series do not exist, and the stale gauge cannot tell a missing pool from an unreachable one), 7. replica skew (**unbounded**, a producer-identity difference: every replica polls, so `max by (pool)` can merge two histories). It also exports `scale_csi_pool_health_last_success_timestamp_seconds`, the driver-owned sample time to use instead of PromQL `timestamp()`. See "Signal timing" in `docs/production.md`. Default off |
