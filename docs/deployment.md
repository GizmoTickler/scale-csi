...
| `backendHealth.enabled` | Run a controller-only **read-only** pool-health poller (`pool.query` + `disk.temperature_alerts`) that fans pool health onto every managed PVC's `VolumeCondition` and exports the `scale_csi_pool_status`/`_healthy`/`_scan_state`/`_scan_errors`/`_disk_temp_alerts`/`_health_stale`/`_health_flip_pending` gauges. The gauges are raw while the condition is debounced, so the two differ transiently — see "Signal timing" in `docs/production.md`. Default off |
| `backendHealth.interval` | Health poll cadence; default `60s`, clamped to `30s`–`2m` (the ceiling keeps a hysteresis-confirmed condition flip inside the 5m `ScaleCSIPoolDegraded` hold). Staleness TTL is 3 x the effective clamped value |
