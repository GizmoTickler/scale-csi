...
| `backendHealth.enabled` | Run a controller-only **read-only** pool-health poller (`pool.query` + `disk.temperature_alerts`) that fans pool health onto every managed PVC's `VolumeCondition` and exports the `scale_csi_pool_status`/`_healthy`/`_scan_state`/`_scan_errors`/`_disk_temp_alerts`/`_health_stale`/`_health_flip_pending` gauges. The gauges are raw, the condition is debounced, and the alerts add their own `for` hold, so they differ transiently in four documented classes — confirmation lag, alert hold and recovery, each with an upper bound, plus poll stall, which is **unbounded**: it lasts until the backend answers. See "Signal timing" in `docs/production.md`. Default off |
