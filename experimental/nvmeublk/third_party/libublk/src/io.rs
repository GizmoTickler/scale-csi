...
/// [`UblkQueue::new`] serves partition `idx` of the queue's tags.
///
...
pub struct UblkDev {
    ...
    /// Run length of the chunked tag partition (1 = interleaved).
    io_tag_chunk: u16,
...
impl UblkDev {
    ...
    /// Chunked interleaving of a queue's tags over its io threads: thread
    /// `k` owns runs of `chunk` consecutive tags, the runs dealt round-robin
    /// (tag `t` belongs to thread `(t / chunk) % n`). `1` (the default) is
    /// the plain interleaved layout. Between the two extremes it keeps a
    /// low-depth submitter on one thread for `chunk` consecutive requests
    /// (blk-mq hands out tags sequentially) while a deeper window still
    /// spans every thread. Clamped when the queue is created to
    /// `depth / n`, so every thread owns at least one run; ignored under
    /// `UBLK_DEV_F_SEQ_TAG_PARTITION`. Set it in the target's init
    /// callback, before the io threads create their queues.
    pub fn set_io_tag_chunk(&mut self, chunk: u16) {
        self.io_tag_chunk = chunk.max(1);
    }

    /// The configured tag chunk (see [`Self::set_io_tag_chunk`]).
    #[inline]
    pub fn io_tag_chunk(&self) -> u16 {
        self.io_tag_chunk
    }

    /// Mark partition `idx` of queue `q_id` as served by the caller.
    /// `-EBUSY` if a live `UblkQueue` already serves it.
    fn claim_partition(&self, q_id: u16, idx: u16) -> Result<PartitionClaim<'_>, UblkError> {
        ...
        match claims.get_mut(slot) {
            Some(taken) if !*taken => {
                *taken = true;
                Ok(PartitionClaim { dev: self, slot })
            }
            Some(_) => {
                log::error!(
...
pub(crate) struct PartitionClaim<'a> {
    dev: &'a UblkDev,
    slot: usize,
}

impl Drop for PartitionClaim<'_> {
    fn drop(&mut self) {
...
pub struct UblkQueue<'a> {
    ...
    nr_tags: u32,
    /// Chunked partition (`UblkDev::set_io_tag_chunk`): run length, or 0
    /// when the partition is the (start, step) progression above.
    tag_chunk: u16,
    /// This thread's index and the io-thread count (chunked partition).
    thread_idx: u16,
    nr_threads: u16,
    /// Hold on this thread's partition slot, released on drop.
    _partition_claim: PartitionClaim<'a>,
    io_cmd_buf: u64,
...
impl UblkQueue<'_> {
    ...
    /// Chunked partition (see [`UblkDev::set_io_tag_chunk`]): the run
    /// length actually used for `depth` tags over `n` threads, or 0 when the
    /// layout is not chunked (one thread, the sequential flag, a chunk of 1,
    /// or no usable run length). It is the largest `c` no bigger than the
    /// requested chunk for which `depth` is a whole number of rounds of `n`
    /// runs (`depth % (c * n) == 0`), so every thread owns exactly the same
    /// number of tags. A partial last round would hand whole extra runs to
    /// the lowest-numbered threads, up to twice the others' share.
    pub(crate) fn effective_chunk(depth: u32, nr_threads: u16, chunk: u16, sequential: bool) -> u16 {
        let n = nr_threads.max(1) as u32;
        if n == 1 || sequential || chunk <= 1 {
            return 0;
        }
        let mut c = (chunk as u32).min(depth / n);
        while c > 1 && depth % (c * n) != 0 {
            c -= 1;
        }
        if c <= 1 {
            0
    ...
    /// Chunked partition of thread `idx`: (first owned tag, one past the
    /// last owned tag, number of owned tags); (0, 0, 0) if it owns none.
    pub(crate) fn chunked_partition(depth: u32, nr_threads: u16, idx: u16, chunk: u16) -> (u16, u16, u32) {
        let mut first = None;
        let (mut last, mut count) = (0u32, 0u32);
        for t in 0..depth {
            if Self::chunk_owns(t, nr_threads, idx, chunk) {
        ...
        match first {
            Some(f) => (f as u16, (last + 1) as u16, count),
            None => (0, 0, 0),
        }
    }

    /// Whether `tag` belongs to thread `idx` under a chunked partition.
    #[inline(always)]
    pub(crate) fn chunk_owns(tag: u32, nr_threads: u16, idx: u16, chunk: u16) -> bool {
        (tag / chunk as u32) % nr_threads as u32 == idx as u32
    }
    ...
    pub fn new_for_thread(
        ...
        log::info!(
            "dev {} queue {} started (io thread {}/{}: {} tags from {} step {} chunk {})",
            dev.dev_info.dev_id,
            q_id,
            thread_idx,
            nr_threads,
    ...
    pub fn tags(&self) -> impl Iterator<Item = u16> + Clone {
        let (start, step, chunk, n, idx) = (self.tag_start, self.tag_step.max(1), self.tag_chunk, self.nr_threads, self.thread_idx);
        (self.tag_start..self.tag_end).filter(move |&t| {
            if chunk > 0 {
                Self::chunk_owns(t as u32, n, idx, chunk)
            } else {
                (t - start) % step == 0
            }
        })
    ...
    pub fn owns_tag(&self, tag: u16) -> bool {
        if tag >= self.tag_end || tag < self.tag_start {
            return false;
        }
        if self.tag_chunk > 0 {
            Self::chunk_owns(tag as u32, self.nr_threads, self.thread_idx, self.tag_chunk)
        } else {
    ...
    pub fn submit_fetch_commands(self, bufs: Option<&Vec<IoBuf<u8>>>) -> Self {
        for i in self.tags() {
            ...
            with_queue_ring_mut_internal!(|ring| {
                self.queue_io_cmd(ring, i, sys::UBLK_U_IO_FETCH_REQ, buf_addr as u64, -1)
            });
        }
        self
    }
    ...
    fn __submit_fetch_commands(&self) {
        for i in self.tags() {
            ...
            with_queue_ring_mut_internal!(|ring| {
                self.queue_io_cmd(ring, i, sys::UBLK_U_IO_FETCH_REQ, buf_addr, -1)
            });
        }
    }
    ...
    pub fn get_inflight_nr_io(&self) -> u32 {
        self.nr_tags - self.state.borrow().get_nr_cmd_inflight()
    }
...
mod tag_partition_tests {
    ...
    /// at least one run (the chunk is clamped to `depth / n`), stay
    /// balanced to within one chunk, and own whole runs of `chunk`
    /// consecutive tags dealt round-robin.
    #[test]
    fn chunked_partition_covers_depth_once() {
        for depth in [4u32, 16, 64, 65, 100, 128, 4096] {
            for n in [2u16, 3, 4, 7, 8, 16] {
                if depth < n as u32 {
                    continue;
                }
                for chunk in [2u16, 3, 4, 8, 16, 32, 1000] {
                    let c = UblkQueue::effective_chunk(depth, n, chunk, false);
                    if c == 0 {
                        // No run length > 1 divides depth into whole rounds:
                        // plain interleave (which is balanced within one tag).
                        assert!((2..=chunk.min((depth / n as u32) as u16)).all(|x| depth % (x as u32 * n as u32) != 0),
                            "depth {depth} n {n} chunk {chunk}: a balanced chunk existed");
                        continue;
                    }
                    assert!(c as u32 <= depth / n as u32, "chunk must leave every thread a run");
                    assert!(c <= chunk, "never larger than requested");
                    assert_eq!(depth % (c as u32 * n as u32), 0, "whole rounds only: depth {depth} n {n} chunk {c}");
                    let mut seen = vec![0u32; depth as usize];
                    let mut sizes = Vec::new();
                    for k in 0..n {
                        let (first, end, count) = UblkQueue::chunked_partition(depth, n, k, c);
                        assert!(count > 0, "depth {depth} n {n} chunk {c}: thread {k} owns nothing");
                        let owned: Vec<u32> = (0..depth).filter(|&t| UblkQueue::chunk_owns(t, n, k, c)).collect();
                        assert_eq!(owned.len() as u32, count);
                        assert_eq!(owned[0], first as u32);
                        assert_eq!(*owned.last().unwrap() + 1, end as u32);
                        for t in &owned {
                            seen[*t as usize] += 1;
                            assert_eq!((*t / c as u32) % n as u32, k as u32);
                        }
                        sizes.push(count);
                    }
                    assert!(seen.iter().all(|&x| x == 1), "depth {depth} n {n} chunk {c}: {seen:?}");
                    let (lo, hi) = (*sizes.iter().min().unwrap(), *sizes.iter().max().unwrap());
                    assert_eq!(hi, lo, "every thread owns the same number of tags: depth {depth} n {n} chunk {c}: {sizes:?}");
                }
            }
        }
        // chunk 1, one thread or the sequential flag: not chunked
        assert_eq!(UblkQueue::effective_chunk(64, 4, 1, false), 0);
        assert_eq!(UblkQueue::effective_chunk(64, 1, 8, false), 0);
        assert_eq!(UblkQueue::effective_chunk(64, 4, 8, true), 0);
        // clamped to depth / n
        assert_eq!(UblkQueue::effective_chunk(64, 4, 32, false), 16);
        assert_eq!(UblkQueue::effective_chunk(64, 4, 4, false), 4);
        // reduced to whole rounds: 128 tags / 3 threads with chunk 32 would be
        // [64, 32, 32]; the largest balanced run length is 1 -> not chunked
        assert_eq!(UblkQueue::effective_chunk(128, 3, 32, false), 0);
        // 96 tags / 3 threads: chunk 32 -> 32 (one whole round); chunk 12 -> 8,
        // the largest run length <= 12 with 96 % (c * 3) == 0
        assert_eq!(UblkQueue::effective_chunk(96, 3, 32, false), 32);
        assert_eq!(UblkQueue::effective_chunk(96, 3, 12, false), 8);
        // 128 tags / 4 threads, chunk 24 -> 16 (128 % 64 == 0), not [48, 32, 24, 24]
        assert_eq!(UblkQueue::effective_chunk(128, 4, 24, false), 16);
        // 64 tags, 4 threads, chunk 4: thread 1 owns 4..8, 20..24, 36..40, 52..56
