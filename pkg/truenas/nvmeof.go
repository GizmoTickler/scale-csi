...
// omit so an unset option reproduces the historical create byte-for-byte.
type NVMeoFSubsystemCreateOptions struct {
	// QidMax sets the maximum number of I/O queues. Nil omits (backend default).
	QidMax *int
	// PiEnable enables T10-PI (protection information). Nil omits (disabled).
	PiEnable *bool
}
...
func (c *Client) NVMeoFSubsystemCreate(ctx context.Context, name string, allowAnyHost bool, hostIDs []int, opts ...NVMeoFSubsystemCreateOptions) (*NVMeoFSubsystem, error) {
	...
	if len(opts) > 0 {
		if opts[0].QidMax != nil {
			params["qid_max"] = *opts[0].QidMax
		}
		if opts[0].PiEnable != nil {
			params["pi_enable"] = *opts[0].PiEnable
		}
	}
...
// NVMeoFPortCreateOptions carries optional port performance fields that default
// to omit so an unset option reproduces the historical create byte-for-byte.
// These live on a port that is SHARED across volumes, so the driver only ever
// sets them from install-wide config — never per-StorageClass.
type NVMeoFPortCreateOptions struct {
	// InlineDataSize sets the inline write cutoff in bytes. Nil omits.
	InlineDataSize *int
	// MaxQueueSize sets the queue entries per connection. Nil omits.
	MaxQueueSize *int
	// PiEnable enables T10-PI on the port. Nil omits.
	PiEnable *bool
}

func (o NVMeoFPortCreateOptions) apply(params map[string]interface{}) {
	if o.InlineDataSize != nil {
		params["inline_data_size"] = *o.InlineDataSize
	}
	if o.MaxQueueSize != nil {
		params["max_queue_size"] = *o.MaxQueueSize
	}
	if o.PiEnable != nil {
		params["pi_enable"] = *o.PiEnable
	}
}
...
func (c *Client) NVMeoFPortCreate(ctx context.Context, transport, address string, port int, opts ...NVMeoFPortCreateOptions) (*NVMeoFPort, error) {
	// Check version support
	if err := c.CheckNVMeoFSupport(ctx); err != nil {
		return nil, err
	}
...
func (c *Client) NVMeoFGetOrCreatePort(ctx context.Context, transport, address string, port int, opts ...NVMeoFPortCreateOptions) (*NVMeoFPort, error) {
	...
	created, err := c.NVMeoFPortCreate(ctx, transport, resolvedAddr, port, opts...)
	if err != nil {
		return nil, err
	}
